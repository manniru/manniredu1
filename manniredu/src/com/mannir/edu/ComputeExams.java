...
public class ComputeExams {

    private static Object[][] sample_data = {
            {"1", "u1131819", "MUHAMMAD MANNIR AHMAD", 5.0, 8.0, 10.0, 5.0, 5.0, 7.0, 6.0},
            {"2", "u1012923", "ZAHRAU SHARU MAIGARI", 4.0, 3.0, 1.0, 3.5, null, null, 4.0},
            {"3", "U1132933", "JAHARA FERAS MACARABON", 4.0, 3.0, 1.0, 3.5, null, null, 4.0},
    };
    ...
    public static void computes() {
    	String[] titles = {"SN", "Student ID",	"Student Name", "Programme", "Level", "Semester", "Module", "Units", "CW", "EXAM", "TOTAL", "GRADE", "POINTS", "GP", "REMARKS"}; 	
    	Db db = new Db("mysql","localhost","root","","manniredu");
    	Object[][]  sample_data = db.regData("registration");
    	
        Workbook wb;

        //if(args.length > 0 && args[0].equals("-xls")) wb = new HSSFWorkbook();else 
        wb = new XSSFWorkbook();
        ...
        titleCell.setCellValue("Student Generated Exam Results");
        ...
        for (int i = 0; i < 10; i++) {
            ...
            for (int j = 0; j < titles.length; j++) {
                ...
                //else if (j == 13){cell.setCellFormula("J" +rownum+ "-K" + rownum); cell.setCellStyle(styles.get("formula")); } 
                else if (j == 14){ cell.setCellFormula("IF(K"+rownum+"<40,\"FAIL\",\"PASS\")"); cell.setCellStyle(styles.get("formula")); } 
                else {
